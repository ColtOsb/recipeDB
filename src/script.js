...
function drawGraph(nodes, edges) {
  ...
  if (edges.length === 0) {
    ...
    const spacingY = Math.min(height / (Math.ceil(nodes.length / cols) + 1), 150);
    nodes.forEach((n, i) => {
    ...
    node.append('circle')
      .attr('r', d => d.node_type === 'Recipe' ? 18 : 12)
      .attr('fill',   d => nodeColor(d.node_type))
      .attr('stroke', d => d3.color(nodeColor(d.node_type)).brighter(1));

    node.append('text')
      .text(d => d.node_name.length > 14 ? d.node_name.slice(0, 14) + '…' : d.node_name)
      .attr('dy', 28);
  ...
  edges.forEach(e => {
    ...
    if (e.source_id === root.node_id && target) {
      childrenMap.get(e.source_id).push({ node: target, edge_type: e.edge_type });
    }
    // If root is the target, source is the child
    else if (e.target_id === root.node_id && source) {
      childrenMap.get(e.target_id).push({ node: source, edge_type: e.edge_type });
    }
  ...
  node.append('circle')
    .attr('r', d => d.data.node_type === 'Recipe' ? 18 : 12)
    .attr('fill',   d => nodeColor(d.data.node_type))
    .attr('stroke', d => d3.color(nodeColor(d.data.node_type)).brighter(1));

  node.append('text')
    .text(d => d.data.node_name.length > 14 ? d.data.node_name.slice(0, 14) + '…' : d.data.node_name)
    .attr('dy', 28);
}
...
async function loadNodes() {
  ...
  if (!search && !type) {
    clearGraph();
    document.getElementById('node-count').textContent = 'Search for nodes above';
    return;
  }
...
async function expandNode(d) {
  document.getElementById('node-count').textContent = `Loading connections for "${d.node_name}"…`;
...
const compoundNodes = [...nodeMap.values()].filter(n => 
  n.node_type === 'Compound_Ingredient' || n.node_type === 'compound_ingredient'
);
...
extraEdgeArrays.flat().filter(e => e.edge_type === 'made_of').forEach(e => {
  // Add the edge
  allEdges.push(e);
  // Add any new nodes it references
  if (!nodeMap.has(e.source_id)) {
    nodeMap.set(e.source_id, {
      node_id:   e.source_id,
      node_name: e.source_name,
